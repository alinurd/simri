...
abstract class REST_Controller extends MX_Controller {
    ...
    protected $controller_method_test = NULL;
    ...
    public function __construct($config = 'rest')
    {
		...
		$this->rest->kelompok_no = -1;
		$this->rest->params_api = null;
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        $this->controller_method_test=$controller_method;
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
            ...
            if ($this->uri->segment(2)=='frontend'){
                $method = str_replace('-','_',$this->uri->segment(3)).'_'.$this->request->method;
            }else{
                $method = str_replace('-','_',$this->uri->segment(2)).'_'.$this->request->method;
            }
            $allow = $this->config->item('allow_lapan_api');
            // dumps($row->kelompok_no);
            // dumps($row);
            // dumps($this->config->item('allow_general_api'));
            // dumps($method);
            if (intval($row->kelompok_no)==-1 && !in_array($method, $allow)){
                return FALSE;
            }
            $allow = $this->config->item('allow_general_api');
            if (intval($row->kelompok_no)==0 && !in_array($method, $allow)){
                return FALSE;
            }
            $allow = $this->config->item('allow_general_api_web');
            if (intval($row->kelompok_no)==2 && !in_array($method, $allow)){
                return FALSE;
            }
            // die();
            ...
            isset($row->kelompok_no) && $this->rest->kelompok_no = $row->kelompok_no;
    ...
    protected function _log_request($authorized = FALSE)
    {
        ...
        $this->rest->params_api= $this->_args;
    ...
    public function post($key = NULL, $xss_clean = NULL)
    {
	 ...
	 public function post_json($key = NULL, $xss_clean = NULL)
    {
        if ($key === NULL)
        {
            return $this->_post_args;
        }
		$post = json_decode($this->_post_args[$key]);
		if(json_last_error() == JSON_ERROR_NONE)
			if (is_array($post))
				$post = implode('', $post);
        return isset($this->_post_args[$key]) ? $this->_xss_clean($post, $xss_clean) : NULL;
    }
