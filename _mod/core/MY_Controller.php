...
class MY_Controller extends MX_Controller {
		...
		foreach($rows as $row){
			...
			if ($config['type_action_button']=='drop'){
				foreach($btn_one as $btn){
					...
					$type='a';
					$url='';
					if (array_key_exists('type', $btn)){$type=$btn['type'];}
					if (array_key_exists('url', $btn)){$url='href="'.$btn['url'].$row[$id].'"';}
	...
	function __export_all($mode='excel'){
		...
		if(method_exists($this->router->fetch_class(), 'printAllAction')){
			$this->printAllAction($data, $title, $mode);
		}else{
			if ($mode=='excel'){
