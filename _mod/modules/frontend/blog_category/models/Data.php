...
class Data extends MX_Model {
	...
	function get_data_posisi_menu(){
		...
		$this->db->where_in('kelompok', ['cat-page', 'cat-blog']);
		$this->db->where('active', 1);
		$this->db->order_by('kelompok,urut');
		$query=$this->db->get();
		$rows=$query->result_array();
		$input=[];
		foreach($rows as $row){
			$input[] = array("id" => $row['id'], "title" => $row['data'], "kelompok" => $row['kelompok'], "slug" => $row['pid'], "pid" => $row['pid'], "urut" => $row['urut'], "active" => $row['active']);
		}
