...
class Data extends MX_Model {
	...
	function get_data_posisi_menu(){
		...
		$this->db->order_by('param_int');
		$this->db->order_by('urut');
		$query=$this->db->get();
		$rows=$query->result_array();
		$input=[];
		foreach($rows as $row){
			$input[] = array("id" => $row['id'], "title" => $row['data'], "slug" => $row['pid'], "pid" => $row['pid'], "urut" => $row['urut'], "active" => $row['active'], "param_string" => $row['param_string'], "param_text" => $row['param_text'], "param_int" => $row['param_int'], "kode" => $row['kode']);
		}
