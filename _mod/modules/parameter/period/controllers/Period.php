...
class Period extends MY_Controller
{
	...
	{
		$this->kelompok_id = 'period';
		$this->set_Tbl_Master( _TBL_COMBO );

		$this->addField( [ 'field' => 'id', 'type' => 'int', 'show' => FALSE, 'size' => 4 ] );
		$this->addField( [ 'field' => 'data', 'title' => 'Periode', 'required' => TRUE, 'search' => TRUE, 'size' => 100 ] );
		...
		$this->addField( [ 'field' => 'list_minggu', 'title' => 'List Minggu', 'type' => 'free', 'mode' => 'a' ] );
		...
		$this->set_Table_List( $this->tbl_master, 'data', 'Periode' );
		...
		if( _MODE_ == 'add' )
		{
			$content_title = 'Penambahan Periode';
		}
		elseif( _MODE_ == 'edit' )
		{
			$content_title = 'Perubahan Periode';
		}
		else
		{
			$content_title = 'Daftar Periode';
		}
	...
	function inputBox_RISK_TYPE( $mode, $field, $row, $value )
	{
		...
		{
			$rows = $this->db->where( 'pid', $row['id'] )->where( 'kelompok', 'term' )->get( _TBL_COMBO )->result_array();
		}
		$content = $this->load->view( 'term', [ 'data' => $rows ], TRUE );
		return $content;
	}

	function inputBox_LIST_MINGGU( $mode, $field, $row, $value )
	{
		if( $mode == 'add' )
		{
			$rows = [];
		...
		{
			$rows = $this->db->where( 'pid', $row['id'] )->where( 'kelompok', 'minggu' )->get( _TBL_COMBO )->result_array();
		}
		$bulan = [];
		for( $x = 1; $x <= 12; ++$x )
		{
			$bulan[$x] = $monthName = date( "F", mktime( 0, 0, 0, $x, 10 ) );
		}
		$content = $this->load->view( 'minggu', [ 'data' => $rows, 'bln' => $bulan ], TRUE );
		return $content;
	}
