...
class Aspek_Risiko extends MY_Controller
{
	...
	{
		$this->kelompok_id = 'aspek-risiko';
		$this->cboDept     = $this->get_combo_parent_dept();

		$this->set_Tbl_Master( _TBL_COMBO );

		$this->addField( [ 'field' => 'id', 'type' => 'int', 'show' => FALSE, 'size' => 4 ] );
		$this->addField( [ 'field' => 'data', 'title' => 'Aktivitas', 'required' => TRUE, 'search' => TRUE, 'size' => 100 ] );
		...
		$this->addField( [ 'field' => 'kriteria_likelihood', 'type' => 'free', 'mode' => 'a' ] );
		...
		if( _MODE_ == 'add' )
		{
			$content_title = 'Penambahan Aspek Risiko';
		}
		elseif( _MODE_ == 'edit' )
		{
			$content_title = 'Perubahan Aspek Risiko';
		}
		else
		{
			$content_title = 'Daftar Aspek Risiko';
		}
	...
	function inputBox_KRITERIA_LIKELIHOOD( $mode, $field, $row, $value )
	{
		if( $mode == 'add' )
		{
			$rows = [];
		...
		{
			$rows = $this->db->where( 'pid', $row['id'] )->where( 'kelompok', 'kriteria-like' )->order_by( 'urut' )->get( _TBL_COMBO )->result_array();
		}
		$content = $this->load->view( 'kriteria', [ 'data' => $rows ], TRUE );
		return $content;
	}
