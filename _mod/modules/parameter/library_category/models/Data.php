	...
	function save_detail($newid=0,$data=array(), $img)
	{
		if (isset($data['edit_id'])){
			if(count($data['edit_id'])>0){
				...
				{
					if (!empty($data['risk_type'][$key])){
						$this->crud->crud_table(_TBL_COMBO);
						$this->crud->crud_field('pid', $newid, 'int');
						$this->crud->crud_field('data', $data['risk_type'][$key]);
						$this->crud->crud_field('kelompok', 'risk-type');
						...
						{
							$title=create_unique_slug($data['risk_type'][$key], _TBL_COMBO);
