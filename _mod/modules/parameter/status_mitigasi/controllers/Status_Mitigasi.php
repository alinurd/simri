...
class Status_Mitigasi extends MY_Controller {
	...
	function init($action='list'){
		$this->kelompok_id='status-mitigasi';
		$this->set_Tbl_Master(_TBL_COMBO);
		
		$this->addField(['field'=>'id', 'type'=>'int', 'show'=>false, 'size'=>4]);
		$this->addField(['field'=>'data', 'title'=>'Status Mitigasi', 'required'=>true, 'search'=>true, 'size'=>100]);
		...
		if (_MODE_=='add') {
			$content_title = 'Penambahan Status Mitigasi';
		}elseif(_MODE_=='edit'){
			$content_title = 'Perubahan Status Mitigasi';
		}else{
			$content_title = 'Daftar Status Mitigasi';
		}
