...
class Data extends MX_Model {
	...
	function save_group($newid=0,$data=array())
	{
		...
		$this->crud->crud_where(['field'=>'user_id', 'value'=>$newid, 'op'=>'=']);
		$this->crud->process_crud();

		if (isset($data['group_no_array'])){
			if(count($data['group_no_array'])>0){
				foreach($data['group_no_array'] as $key=>$row)
				{
					$this->crud->crud_table(_TBL_USERS_GROUPS);
					$this->crud->crud_type('add');
					$this->crud->crud_field('group_id', $row, 'int');
					$this->crud->crud_field('user_id', $newid, 'int');
