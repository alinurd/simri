            ...
            { ?>
                <tr>
                    ...
                    <td class="text-center">
                        <a href="#" data-toggle="collapse" data-target="#row<?= $vReg["id"] ?>" aria-expanded="false"
                            aria-controls="row<?= $vReg["id"] ?>" class="btn bg-success btn-sm">
                            <b><i class="icon-arrow-down16"></i></b>
                        </a>
                    </td>
                    <td class="text-center">
                        <button type="button" id="btn-reg-edit" data-id="<?= $vReg["id"] ?>" data-url="<?= $btnEdit ?>"
                            class="btn btn-labeled button-action bg-primary btn-sm"><i class="icon-pencil"></i></button>
                    </td>
                </tr>
                                                                ...
                                                                { ?>
                                                                    <tr>
                                                                        ...
                                                                        { ?>
                                                                            ...
                                                                            <td rowspan="<?= $spanCount; ?>" class="text-center">
                                                                                <button type="button" id="add-monitoring" data-url="<?= $btnAdd ?>"
                                                                                    data-kajian-id="<?= $vReg['id_kajian_risiko'] ?>"
                                                                                    data-mitigasi-id="<?= $vMonitoring['id_mitigasi'] ?>"
                                                                                    class="btn btn-labeled button-action bg-success btn-sm add-monitoring"><i
                                                                        ...
                                                                        <td class="text-center">
                                                                            <?= ( ! empty( $vMonitoring['tanggal_update'] ) && date( "d-m-Y", strtotime( $vMonitoring["tanggal_update"] ) ) != "00-00-0000" ) ? date( "Y-m-d", strtotime( $vMonitoring['tanggal_update'] ) ) : "" ?>
                                                                        </td>
                                                                        <td class="text-center">
                                                                            <?php if( ! empty( $vMonitoring["dokumen_pendukung"] ) && file_exists( "./files/kajian_risiko_monitoring/{$vMonitoring["dokumen_pendukung"]}" ) ) : ?>
                                                                                <a href="<?= base_url( "files/kajian_risiko_monitoring/{$vMonitoring["dokumen_pendukung"]}" ) ?>"
                                                                                    target="_blank"><i class="icon-file-text"></i></a>
                                                                            <?php endif; ?>
                                                                        </td>
                                                                        <td><?= $vMonitoring['status'] ?></td>
                                                                        <td class="text-center">
                                                                            <?php if( ! empty( $vMonitoring["id_monitoring"] ) ) : ?>
                                                                                <button type="button" id="btn-edit-monitoring"
                                                                            ...
                                                                            <?php endif; ?>
                                                                            <?php if( ! empty( $vMonitoring["id_monitoring"] ) ) : ?>
                                                                                <button type="button" id="btn-delete-monitoring"
                                                                                    data-kajian-id="<?= $vReg['id_kajian_risiko'] ?>"
                                                                                    data-id="<?= $vMonitoring["id_monitoring"] ?>"
                                                                                    data-url="<?= $btnDelete ?>"
