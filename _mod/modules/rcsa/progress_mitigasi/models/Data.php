...
class Data extends MX_Model
{
	...
	function update_list_indi_like( $data = [], $save = "" )
	{
		...
		$this->db->where( 'like_code', intval( $code ) );
		$this->db->where( 'impact_code', intval( $data['dampak_id'] ) );
		...
		if( $save )
		{
			...
			$this->crud->crud_field( 'rcsa_detail_id', $data['rcsa_detail_no'] );
			$this->crud->crud_field( 'score_like', $data['score'] );
			$this->crud->crud_field( 'like', $rows['like_code'] );
			$this->crud->crud_field( 'impact', $rows['impact_code'] );
			if( $p )
			{
				$this->crud->crud_type( 'edit' );
				$this->crud->crud_where( [ 'field' => 'id', 'value' => $p['id'] ] );
	...
	function simpan_like_indi( $data )
	{
		...
		$hasil       = $this->update_list_indi_like( $data, TRUE );
		$hasil['id'] = $id;
		return $hasil;
	}
