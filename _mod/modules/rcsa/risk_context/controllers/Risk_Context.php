...
class Risk_Context extends MY_Controller
{
	...
	function identifikasi_content($data = [], $parent = [])
	{
		...
		if ($data) {
			...
			foreach ($pi as $key => $x) {
				...
				$penyebab .= '<tr><td style="padding-left:0px;">' . form_dropdown('penyebab_id[]', $penyebab_id, $x, 'id="penyebab_id_"  class="form-control select" style="width:100%;"') . form_input('penyebab_id_text[]', '', 'class="form-control d-none" id="penyebab_id" placeholder="' . _l('fld_penyebab_risiko') . '" ') . '</td><td class="text-right pointer" width="10%" style="padding-right:0px;">' . $icon . '</td></tr>';
			}
		} else {
			$penyebab .= '<tr><td style="padding-left:0px;">' . form_dropdown('penyebab_id[]', $penyebab_id, '', 'id="penyebab_id_"  class="form-control select" style="width:100%;"') . form_input('penyebab_id_text[]', '', 'class="form-control d-none" id="penyebab_id_text" placeholder="' . _l('fld_penyebab_risiko') . '"') . '</td><td class="text-right pointer" width="10%" style="padding-right:0px;"><i class="icon-plus-circle2 text-primary-400 add-penyebab"></i></td></tr>';
		}
    ...
    <tr>
		...
		// dropdown kuantitatif
		// $param['analisa_kuantitatif'][] = [ 'title' => _l( 'fld_likelihood' ), 'help' => _h( 'help_likelihood' ), 'mandatori' => TRUE, 'isi' => form_dropdown( 'like_id', $like, ( $data ) ? $data['like_id'] : '', 'id="like_id" class="form-control select" style="width:100%;"' ) . form_hidden( [ 'like_text_kuantitatif' => ( $data['like_inherent'] ) ? $data['like_inherent'] : '' ] ) . form_hidden( [ 'like_id_2' => ( $data ) ? $data['like_id'] : '' ] ) ];

		// $param['analisa_kuantitatif'][] = [ 'title' => _l( 'fld_impact' ), 'help' => _h( 'help_impact' ), 'mandatori' => TRUE, 'isi' => form_dropdown( 'impact_id', $impact, ( $data ) ? $data['impact_id'] : '', 'id="impact_id" class="form-control select" style="width:100%;"' ) . form_hidden( [ 'impact_text_kuantitatif' => ( $data['impact_inherent'] ) ? $data['impact_inherent'] : '' ] ) . form_hidden( [ 'impact_id_2' => ( $data ) ? $data['impact_id'] : '' ] ) ];
