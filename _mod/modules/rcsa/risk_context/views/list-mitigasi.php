...
<div class='table-responsive'>
    ...
    <?php if( $rcsa_detail['treatment_id'] != 1 && $rcsa_detail['efek_mitigasi'] != 4 ) : ?>
        <span <?= $no_edit; ?> class="btn bg-primary-400 btn-labeled btn-labeled-right legitRipple pull-right"
            data-id="<?= $rcsa_detail['id']; ?>" id="add_mitigasi" style="pointer-events:<?= $events; ?>"><b><i
                    class="icon-file-plus "></i></b> <?= _l( 'fld_add_mitigasi' ); ?></span>
    <?php endif ?>
    <br />&nbsp;<br />&nbsp;
            ...
            foreach( $mitigasi as $row ) :
                ...
                <tr>
                    ...
                    <td class="pointer text-center">
                        <i class="icon-database-edit2  text-primary-400 update-mitigasi <?= $no_edit_hide; ?>"
                            data-rcsa="<?= $rcsa_detail['id']; ?>" data-id="<?= $row['id']; ?>" data-popup="tooltip"
                            data-html="true" title=" Update Mitigasi "></i> <?= $del; ?>
                    </td>
                    <td class="pointer text-center">
                        <span class="btn bg-primary-300 add_aktifitas_mitigasi pointer" data-id="<?= $row['id']; ?>"
                            data-popup="tooltip" data-html="true" title=" Tambah Aktifitas Mitigasi "> <i
                                class="icon-file-plus "></i> Aktifitas </span>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
