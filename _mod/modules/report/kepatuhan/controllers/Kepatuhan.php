...
class Kepatuhan extends MY_Controller
{
	...
	function content($ty = 'detail')
	{
		...
		// $minggu=$this->crud->combo_select(['id', 'param_string as minggu'])->combo_where('kelompok', 'minggu')->combo_tbl(_TBL_COMBO)->get_combo()->result_combo();
		// Doi::dump($minggu);
		unset($minggu[""]);
	...
	function get_detail()
	{
		...
		if ($post['term'] == 'xixi') {
			$data = $this->data->get_data_lap_all($post['tahun'], $post['term'], 0, $post['divisi'], $tgl);
		} else {
		...
		$data['tahun'] = $post['tahun_t'];
		$data['is_triwulan'] = (strpos(strtolower($post['term_t']), 'triwulan') !== false) ? 1 : 0;

		$terms = $this->db->where('kelompok', 'term')->where('pid', $post['tahun'])->get(_TBL_COMBO)->result_array();

		$selectedTerms = [];
		foreach ($terms as  $ter) {
			$minggu = $this->db->where('kelompok', 'minggu')->where('param_date>=', $ter['param_date'])->where('param_date<=', $ter['param_date_after'])->get(_TBL_COMBO)->result_array();
			$selectedTerms[$ter['data']] = [];
			foreach ($minggu as $m) {
				$selectedTerms[$ter['data']][$m['id']] = $m['param_string'];
			}
		}

		$data['all_triwulan'] = $selectedTerms;

		if ($post['term'] == 'xixi') {
			$hasil['detail'] = $this->load->view('detail_all', $data, true);
			$hasil['detail2'] = $this->load->view('detail2_all', $data, true);
		} else {
