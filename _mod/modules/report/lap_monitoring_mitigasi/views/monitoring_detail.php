    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-header header-elements-sm-inline">
                <?php if( isset( $back ) ) : ?>
                    <span class="btn bg-warning-400 btn-labeled btn-labeled-left legitRipple" data-id="<?= $id ?>"
                        id="back_list"><b><i class="icon-arrow-left5"></i></b> Kembali</span>
                <?php endif; ?>


                <a target="_blank"
                    href="<?= base_url( '/lap-monitoring-mitigasi/cetak-register/' . $pos['period'] . '/' . $pos['owner'] . '/' . $pos['type_ass'] ) ?>">
            ...
            <div class="card-body">
                <table class="table table-borderless">
                    <tr>
                        ...
                        <td><strong><?= ! empty( $parent['owner_name'] ) ? $parent['owner_name'] : ""; ?></strong></td>
                    </tr>
                    <tr>
                        <td><em>Sasaran Departmen</em></td>
                        <td><strong><?= ! empty( $parent['sasaran_dept'] ) ? $parent['sasaran_dept'] : ""; ?></strong>
                        </td>
                    </tr>
                    <tr>
                        <td><em>Periode</em></td>
                        <td><strong><?= ! empty( $parent['period_name'] ) ? $parent['period_name'] . ' - ' . $parent['term'] : ""; ?></strong>
                        </td>
                    </tr>
                </table>
                        ...
                        <tbody>
                            ...
                            <?php foreach( $rows as $row ) : ?>
                                ...
                                <tr>
                                    ...
                                    <td rowspan="<?= ( $jml > 0 ) ? $jml : $jml_ident; ?>"><?= $row['penyebab_risiko']; ?>
                                    </td>
                                    <td rowspan="<?= ( $jml > 0 ) ? $jml : $jml_ident; ?>"
                                        style="background-color:<?= $row['color']; ?>;color:<?= $row['color_text']; ?>;">
                                        <?= $row['level_color']; ?>
                                    </td>
                                    ...
                                    if( array_key_exists( $row['id'], $mitigasi ) )
                                    {
                                        if( $mitigasi[$row['id']] )
                                        {
                                            foreach( $mitigasi[$row['id']] as $key => $mit ) :
                                                ...
                                                <td><?= ( $mit['batas_waktu_tindak_lanjut'] != '1970-01-01' ) ? $mit['batas_waktu_tindak_lanjut'] : "-"; ?>
                                                </td>
                                                <?php
                                                if( $key == 0 ) : ?>
                                                    <td>-</td>
