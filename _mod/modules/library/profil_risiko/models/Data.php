...
class Data extends MX_Model
{
	...
	function filter_data_mon($dtuser)
	{
		...
		if ($this->cek_tgl) {
			...
			if (isset($this->pos['minggu']) && intval($this->pos['minggu'])) {
			...
			if (!isset($this->pos['tgl1']) && isset($this->pos['term_mulai']) && intval($this->pos['term_mulai'])) {
				$rows = $this->db->select('*')->where('id', intval($this->pos['term_mulai']))->get(_TBL_COMBO)->row_array();
				$this->pos['tgl1'] = $rows['param_date'];
			}


			if (!isset($this->pos['tgl2']) && isset($this->pos['term_akhir']) && intval($this->pos['term_akhir'])) {
		...
		if ($this->pos) {
			...
			if (isset($this->pos['tgl1']) && isset($this->pos['tgl2'])) {
