...
class Left_Sidebar extends Widget
{
	...
	function _menu_kiri( $ad )
	{
		...
		$active = '';
		$except = [ "profil-risiko" ];
		if( in_array( _MODULE_NAME_, $except ) )
		{
			if( $ad["nm_modul"] == "profil-risiko/dashboard" && _MODE_ == "dashboard" )
			{
				$active = "active";
			}
			elseif( $ad["nm_modul"] == "profil-risiko" && _MODE_ == "list" )
			{
				$active = "active";
			}
		}
		...
		{
			if( $ad['nm_modul'] == _MODULE_NAME_ && ! in_array( $ad["nm_modul"], $except ) && ! in_array( _MODULE_NAME_, $except ) )
			{
				$active = 'active';
			}
		}
