...
<table class="table table-hover table-striped " id='datatable-list'>
    <thead>
        <tr>
            <th width='5%' class='text-center' style="padding-left:2px;"><input type="checkbox"
                    class="form-check-input pointer" name="chk_list_parent" id="chk_list_parent"
                    style="padding:0;margin:0;"></th>
            <th width='5%' class='text-center'>#</th>
            <?php
            foreach( $title as $row ) : ?>
                <th data-orderable="<?= ( $row[6] ) ? "true" : "false" ?>" class='text-<?= $row[4]; ?> <?= $row[5]; ?>'
                    width='<?= $row[3]; ?>%'>
                    <?= $row[2]; ?>
                </th>
            <?php endforeach; ?>
