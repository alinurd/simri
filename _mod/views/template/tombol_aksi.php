			...
			<div class="col-xl-12">
				...
				foreach($rows as $key=>$row):
					...
					<?php else:
						...
						?>
						<a type="<?=$type;?>" id="<?=$row['id'];?>" name="<?=(isset($row['name']))?$row['name']:$row['id'];?>"  class="btn <?=$row['color'];?> btn-labeled btn-labeled-left  <?=$float.' '.$row['round'];?> <?=(isset($row['class']))?$row['class']:' button-action';?>" href="<?=$row['url'];?>" style="margin-right:5px;" <?=(isset($row['attr']))?$row['attr']:'';?> >
							<b><i class="<?=$row['icon'];?>"></i></b> <?=$row['label'];?>
						</a>
						<?php else:?>
						<button type="<?=$type;?>" id="<?=$row['id'];?>" name="<?=(isset($row['name']))?$row['name']:$row['id'];?>"  class="btn <?=$row['color'];?> btn-labeled btn-labeled-left <?=$float.' '.$row['round'];?> <?=(isset($row['class']))?$row['class']:' button-action';?>" data-url="<?=$row['url'];?>" style="margin-right:5px;" <?=(isset($row['attr']))?$row['attr']:'';?> value="<?=(isset($row['value']))?$row['value']:'x';?>">
							<b><i class="<?=$row['icon'];?>"></i></b> <?=$row['label'];?>
						</button>
					<?php endif;
					endif;?>
