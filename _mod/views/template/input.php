...
foreach ($tabs as $row):?>
    <li class="nav-item">
        <a href="#content-<?=$row['id'];?>" class="nav-link <?=($awal)?'active show':'';?>" data-toggle="tab"><?=$row['title'];?></a>
    </li>
<?php $awal=false;
endforeach;?>
</ul>
    ...
    foreach ($tabs as $row):
        ...
        if (array_key_exists('cols', $row)){
                ...
                foreach ($cols as $col):
                    if (isset($fields[$col]['box'])):
                        ...
                        <div class="form-group row">
                            <?php
                            if ($fields[$col]['line']):?>
                                <legend class="text-uppercase font-size-lg text-primary font-weight-bold"><i class="<?=$fields[$col]['line-icon'];?>"></i> <?=$fields[$col]['line-text'];?></legend>
                            <?php endif;?>
                            <label class="col-form-label col-lg-2 text-<?=$params['align_label'];?>"><?=($fields[$col]['required'])?$required:''?><?=$fields[$col]['title'];?>
                                <?=$span_help;?>
                            </label>
                            <div class="col-lg-10">
                                <?php
                                ...
                                echo $box;
                                ?>
                                <span class=" text-muted" id="help_<?=$fld;?>"><?=$help;?></span>
            ...
            foreach ($row['field'] as $fld):
                ...
                <?php
                if ($x['line']):?>
                    <legend class="text-uppercase font-size-lg text-primary font-weight-bold"><i class="<?=$x['line-icon'];?>"></i> <?=$x['line-text'];?></legend>
                <?php endif;?>
                <div class="form-group row">
                    <?php 
                    if ($x['mode']=='o'):?>
                        <label class="col-form-label col-lg-2 text-<?=$params['align_label'];?>"><?=($x['required'])?$required:''?><?=$x['title'];?>
                            <?=$span_help;?> 
                        </label>
                    <?php $col2=10; 
                    else:?>
                        <label class="col-form-label col-lg-12 text-left"><?=($x['required'])?$required:''?><?=$x['title'];?>
                            <?=$span_help;?> 
                        </label>
                    <br/>
                    <?php $col2=12; endif;?>
