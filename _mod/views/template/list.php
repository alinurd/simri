...
endforeach;
if ($search)
    echo 'Search by : ' . $filter . ' <a href="'.base_url(_MODULE_NAME_).'?cs=1"> <i class="icon-close2 pointer text-danger"></i></a><hr>';
?>
<table class="table table-hover table-striped " id='datatable-list'>
    <thead>
        <tr>
            ...
            <th class='text-<?=$row[4];?>' width='<?=$row[3];?>%'><?=$row[2];?></th>
            <?php endforeach;?>
...
<div id="modal_form_search" class="modal fade" tabindex="-1">
...
<script>
    ...
    var _componentDatatableAdvanced = function() {
        ...
        $('#datatable-list tbody').on('mouseover', 'td', function() {
